...
public class UserRegistrationServiceTest {
    ...
    void loginSuccess(){
        when(this.repository.findByUsername(requestLogin.login())).thenReturn(Optional.ofNullable(userEntity));
        ...
        verify(this.repository, times(1)).findByUsername(requestLogin.login());
    }

    @Test
    @DisplayName("Teste de erro ao inserir senha errada")
    void loginErrorPass() {
        when(this.repository.findByUsername(requestPasswordInvalid.login())).thenReturn(Optional.ofNullable(userEntity));
        ...
        verify(this.repository, times(1)).findByUsername(requestPasswordInvalid.login());
    }
    ...
    void loginErrorNotFound(){
        ...
        verify(this.repository, times(1)).findByUsername(requestLoginInvalid.login());
    }
    ...
    void registerSuccess() {
        ...
        when(this.repository.findByUsername(requestRegister.login())).thenReturn(Optional.empty());
        ...
        assertNotNull(result);
        verify(this.repository, times(1)).findByUsername(requestRegister.login());
    ...
    void registerError() {
        when(this.repository.findByUsername(requestRegister.login())).thenReturn(Optional.ofNullable(userEntity));

        var result = assertThrows(UserAlreadyExistsException.class,
                () -> this.userRegistrationService.register(requestRegister));

        assertEquals(String.format("User with login %s already exists.", userEntity.getUsername()), result.getMessage());
        verify(this.repository, times(1)).findByUsername(requestRegister.login());
    }
}
